...
TYPED_TEST(GenEigensolverTestCapi, CorrectnessDistributedDLAFWithRedistribution) {
  using T = TypeParam;
  ...
  for (comm::CommunicatorGrid& grid : this->commGrids()) {
    ...
    for (auto uplo : blas_uplos) {
      for (auto [m, mb] : redistribution_sizes) {
        testGenEigensolver<T, API::dlaf, do_factorization>(dlaf_context, uplo, m, mb, grid,
                                                            std::nullopt);
        testGenEigensolver<T, API::dlaf, already_factorized>(dlaf_context, uplo, m, mb, grid,
                                                              std::nullopt);
      }
    }
